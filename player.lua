--give it health
--make it take dmg and bounce enemies off it like the player
Crystal = Object:extend()
Crystal:implement(GameObject)
Crystal:implement(Physics)
Crystal:implement(Unit)
function Crystal:init(args)
  self:init_game_object(args)
  self:init_unit()

  self.color = white[0]
  self:set_as_rectangle(9,9,'static','crystal')
...
function Crystal:on_collision_enter(other, contact)
...
function Crystal:hit(damage, from_undead)
  ...
  if self.hp <= 0 then
    --do death state
    --end arena go to some other scene
    self.dead = true
  end
end
...
function Player:update(dt)
  ...
  if self.leader then
    ...
    local new_vx = 0
    local new_vy = 0
    if input.move_left.down then new_vx = new_vx - 100 end
    if input.move_right.down then new_vx = new_vx + 100 end
    if input.move_up.down then new_vy = new_vy - 100 end
    if input.move_down.down then new_vy = new_vy + 100 end

    self:set_velocity(new_vx, new_vy)

    --ok after this make followers not a thing
    --and figure out how to make UI happen
    --figure out what u wanna do with units, maybe be able to place them around or something
...
function Gold:init(args)
  ...
  --should make magnet really shit
  --self.weak_magnet_sensor = Circle(self.x, self.y, 16)
  --self.magnet_sensor = Circle(self.x, self.y, 56)
  self.weak_magnet_sensor = Circle(self.x, self.y, 1)
  self.magnet_sensor = Circle(self.x, self.y, 2)
end
...
function Gold:on_trigger_enter(other, contact)
  ...
  if other:is(Player) then
    ...
    --then update the gold text
    main.current.gold_text:set_text{{text = '[fg, nudge_down]gold: [white, nudge_down]' .. main.current.gold_picked_up, font = pixul_font, alignment = 'center'}}
    --do the flash effect for the gold pickup
    trigger:after(0.025, function()
      main.current.gold_text:set_text{{text = '[wavy_mid]gold: [white_to_yellow]' .. main.current.gold_picked_up, font = pixul_font, alignment = 'center'}}
    end)
